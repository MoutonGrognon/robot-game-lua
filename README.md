...
- [make](https://www.gnu.org/software/make/). It is optionnal but will make your life easier (you can also launch all the command from the Makefile manually)
- an API client such as [bruno](https://www.usebruno.com/) (a bruno collection is provided in this project under the flolder `bruno`). It is optionnal but recommanded because some of the endpoints are not covered by the frontend
...
The automatch system should start running ranked matches automatically.

You can use the front-end to see the matches with your browser at `localhost:5555`.

Then, if you want to create unranked matches between the bots of your choice, open your API Client.
...
_Example: `{actionType=ATTACK, x=8, y=9}` is a valid return value._
...
- `location` : The location of the robot as a Location with the properties `x` and `y`corresponding to its x and y coordinates. _Note: a Location can be directly compared to another Location, for example `self.location == rg.CENTER_POINT` will return `true` if the bot is at the center of the arena._
...
There are no friendly damage (collision, attack and suicide only deal damage to enemy robots).
